...
### This is an analysis on how a bypass in Microsoft Outlook's "Protected View" allows attackers to force NTLM authentication to a remote server.

During this lab I learned how a single character (an exclamation point) turns a simple link into an automated "ID thief". By exploiting a bypass that tricks Outlook's usual security checks through an exclamation point embedded in an email link (**"!"**). I managed to bypass 'protected view' completely. This let me force a user's computer to hand over its NTLM hashes without the user ever knowing. Hence why the vulnerablity was rated as an **9.8 critical vulnerability.**
...
> **Note:** **COM** stands for **Component Object Model**. Developed by Microsoft as a "language-neutral, binary-interface standard" allowing different software components to communicate and interact despite the programming language that is used.

In my own words, a **Moniker** is a **"Smart-Link"**. As a normal link just points to a file, a moniker link points to that file as well as carrying instructions on how that file should be opened or **"Bound"** as the Microsoft definition states. 
